    ...
    <>
      {tieredUp ? (
        <div className="text-center rounded-[18px] p-4 mb-3.5 text-white animate-pop relative overflow-hidden" style={{ background: 'linear-gradient(135deg,var(--gj-navy),#123e69)' }}>
          ...
          <div className="relative">
            <div className="text-[48px] animate-pop" aria-hidden="true">{after.tier.icon}</div>
            <h4 className="m-0 mt-1.5 text-lg font-extrabold tracking-tight">TIER UP — you're now {after.tier.name}!</h4>
            <p className="m-0 text-[12.5px] text-white/85 leading-snug mt-1">{after.tier.unlocks}</p>
            {newlyUnlocked > 0 && <div className="inline-block mt-2.5 text-[12px] font-bold bg-red rounded-full px-3 py-1 animate-pop">🔓 {newlyUnlocked} new formal job{newlyUnlocked > 1 ? 's' : ''} unlocked</div>}
          </div>
        </div>
      ) : (
      ...
      {newBadges.length > 0 && (
        <div className="text-center mb-3">
          <b className="text-[13px] text-navy">🏅 New badge{newBadges.length > 1 ? 's' : ''} unlocked!</b>
